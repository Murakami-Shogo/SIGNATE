...
def get_model_max_len(tokenizer, model) -> int:
    """
    安全な最大長を返す。
    - tokenizer.model_max_length を優先（多くの場合 512 が入っている）
...
def main():
    ...
    log_file_abs = os.path.join(logs_dir_abs, f"embedding_1v1_{stamp}_{model_slug}.log")
    setup_logging(log_file_abs)

    logging.info("=== embedding_1v1 start ===")
    ...
    sim = test_emb @ base_emb.T  # (N, 50)
    top2_idx = torch.topk(sim, k=2, dim=1, largest=True).indices.numpy()

    pred_a = base_ids[top2_idx[:, 0]]
    pred_b = base_ids[top2_idx[:, 1]]

    sub_path_abs = os.path.join(output_dir_abs, f"submission_{stamp}_{model_slug}.csv")
    sub_df = pd.DataFrame({"id": test_ids, "pred_a": pred_a, "pred_b": pred_b})
