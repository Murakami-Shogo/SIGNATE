...
- 元データ（train/test）に加えて、国土数値情報の「駅別乗降者数」データを取り込み、立地情報を強化している。
- モデルは CatBoost + ログ変換 + Optuna でハイパラチューニング。
...
- 国土数値情報の駅データ（2019–2023年）を shapefile から読み込み。
...
- 各物件について:
  ...
  - 各物件を中心に **800m のバッファ（円）** を作り、駅ポイントと空間結合。
  - 800m 圏内にある駅について
